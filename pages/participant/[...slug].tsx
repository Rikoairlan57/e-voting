  ...
  useEffect(() => {
    if (vote && participant) {
      const candidate = vote?.candidates?.find(
        (c) => c.name === participant?.candidate
      );
      if (candidate) {
          ...
          {!participantLoading &&
            vote?.candidates?.map((candidate: Candidate, index: number) => (
              <CandidateItem
                ...
                percentage={
                  candidate.votes
                    ? (candidate.votes / vote?.totalVotes) * 100
                    : 0
                }
              />
            ))}
        ...
        <div className="text-center mt-10">
          ...
          {participant && (
            <span className="bg-zinc-100 py-2 px-3">
              You have chosen, and are not allowed to change choice
            </span>
          )}
          {session?.user?.email === vote?.publisher && (
            <span className="bg-zinc-100 py-2 px-3">Voters cannot vote</span>
          )}
        </div>
      </div>
    </div>
